/**
 * `embed --stale` signature reconciliation 회귀 테스트.
 *
 * 페이지 하나가 2,000행 stale cursor 경계를 가로지를 수 있으므로 signature는
 * cursor batch의 부분집합이 아니라 최종 DB 상태를 기준으로 기록해야 한다.
 *
 * process-global gateway transport seam을 설치하므로 `.serial.test.ts`로 둔다.
 * transport는 항상 fake이며 이 파일에서는 외부 embedding을 호출하지 않는다.
 */
